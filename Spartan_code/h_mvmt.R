...
bite <- function(parms, domain, state, nBites) {
  with(as.list(c(parms, domain, state)), {
    type_M <- runif(1)
    type_H <- runif(1)
    success <- runif(1)
    ...
    state <- list(t=t, S_H=S_H, I_H=I_H, R_H=R_H, S_M=S_M, I_M=I_M, 
                  N_H=N_H, N_M=N_M)
    return(list(domain=domain, state=state, nBites=nBites))
  })
}
  ...
  with(parms, {
    ...
    while (state$t < t_max) {
      ...
      if (event*R < state$I_H*gamma_H) { # human recovery
        ...
        out <- bite(parms, domain, state, nBites)
        domain <- out$domain
        state <- out$state
        nBites <- out$nBites
      }
...
run_rep <- function(i, parms) {
  ...
  # extracting values at fixed times seq(0,100)
  S_H_vec <- numeric(101)
  I_H_vec <- numeric(101)
  S_M_vec <- numeric(101)
  I_M_vec <- numeric(101)
  ...
  return(list(S_H=S_H_vec, I_H=I_H_vec, S_M=S_M_vec, I_M=I_M_vec,
              nBites=out$nBites)) 
}
...
# various combinations of resolution and human diffusion
D_H <- commandArgs(trailingOnly=TRUE)
D_H <- as.numeric(D_H)
print(D_H)
...
for (res in 1:20) {
  ...
  shDF <- data.frame(matrix(nrow=101, ncol=num_reps))
  ihDF <- data.frame(matrix(nrow=101, ncol=num_reps))
  smDF <- data.frame(matrix(nrow=101, ncol=num_reps))
  imDF <- data.frame(matrix(nrow=101, ncol=num_reps))
  bite_vec <- numeric(num_reps)
  for (j in 1:num_reps) {
    singleRun <- out[[j]]
    shDF[,j] <- singleRun$S_H
    ihDF[,j] <- singleRun$I_H
    smDF[,j] <- singleRun$S_M
    imDF[,j] <- singleRun$I_M
    bite_vec[j] <- singleRun$nBites
  }
  medSH <- apply(shDF, 1, median)
  medIH <- apply(ihDF, 1, median)
  medSM <- apply(smDF, 1, median)
  medIM <- apply(imDF, 1, median)
  medBite <- median(bite_vec)
  # save 
  save_file <- paste0('outputs/D', D_H, 'res', res, '.RData')
  save(medSH, medIH, medSM, medIM, medBite, file=save_file)
}
